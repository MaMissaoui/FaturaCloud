...
// order_default.xlsx, incoming_invoice_default.xlsx, …) — see each
// buildXTemplate function's own doc comment (invoice.go, purchase_order.go,
// order.go, incoming_invoice.go) for that document type's layout. A real
// Excel/LibreOffice session wasn't available in the environment this feature
// was built in, so each output is built programmatically via excelize rather
// than hand-authored — every output is still a real, valid .xlsx a user can
// open and edit like any other.
//
