...
func TestDeleteStockMovementBlockedForShippedDelivery(t *testing.T) {
	...
	product, err := d.CreateProduct(CreateProductRequest{
		ID: "prod-del-mv-1", OrganizationID: org.ID, Name: "Widget", Type: "product", StockEnabled: 1,
	})
	if err != nil {
		t.Fatalf("CreateProduct: %v", err)
	}
	...
	outMovement := findMovementByReference(movements, "DEL-MV-1", "out")
	if outMovement == nil {
		t.Fatalf("expected an \"out\" movement referencing DEL-MV-1, got %+v", movements)
	}

	_, err = d.DeleteStockMovement(outMovement.ID)
	if err == nil {
		t.Fatal("expected deleting a movement generated by a shipped delivery to be rejected")
	}
...
func TestDeleteStockMovementBlockedForReceivedReceipt(t *testing.T) {
	...
	_, err := d.DeleteStockMovement(movements[0].ID)
	if err == nil {
		t.Fatal("expected deleting a movement generated by a received receipt to be rejected")
	}
	var verr *ValidationError
	if !errors.As(err, &verr) {
		t.Fatalf("expected a *ValidationError, got %T: %v", err, err)
