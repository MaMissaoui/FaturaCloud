...
// The delivery form edits deliveryDate as a dayjs object, not the wire's
// unix-ms timestamp.
type DeliveryFormValues = Omit<Partial<Delivery>, "deliveryDate"> & {
  deliveryDate?: Dayjs | number;
  lineItems?: Partial<DeliveryLineItem>[];
};
...
export const deliveryAtom = atom(
  ...
  async (get, set, newValues: DeliveryFormValues) => {
    const deliveryId = get(deliveryIdAtom);
...
export const deleteDeliveryAtom = atom(null, async (get, set, deliveryId: string) => {
  try {
    ...
    if (success) {
      const list = reject(get(deliveriesAtom), (d) => isEqual(d.id, deliveryId));
