...
const DeliveryNotePDF = ({ order, lineItems, client, organization, locale }: Props) => {
  const dateLocale = locale ?? "en";
  const showPrices = lineItems.some((item) => (item.unitPrice ?? 0) > 0);
      ...
      <Document>
        <Page size="A4" style={styles.page}>
          ...
          <View style={styles.tableHeader}>
            ...
            {showPrices && <Text style={[styles.colPrice, styles.headerText]}>Unit Price</Text>}
            {showPrices && <Text style={[styles.colTotalBold, styles.headerText]}>Total</Text>}
          </View>
          ...
          {lineItems.map((item, idx) => {
            ...
            const fmt = (n: number) =>
              Intl.NumberFormat(dateLocale, { style: "currency", currency }).format(n / 100);
            ...
            return (
              <View key={item.id ?? idx} style={styles.tableRow}>
                ...
                {showPrices && <Text style={styles.colPrice}>{fmt(price)}</Text>}
                {showPrices && <Text style={styles.colTotalBold}>{fmt(total)}</Text>}
              </View>
            );
          })}
