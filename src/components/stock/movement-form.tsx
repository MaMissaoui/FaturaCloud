...
const MovementForm = () => {
  ...
  const isSerialized = !!selectedProduct?.serialized;
  const inStockSerials = filter(
    serialNumbersByProduct[selectedProductId] ?? [],
    (s: any) => s.inStock,
  );
  ...
  // A serialized product only supports in/out — coerce away from whatever
  // type was selected before rather than just filtering the options, or a
  // stale adjustment/count type would silently reach the server and 409.
  useEffect(() => {
    if (!isSerialized) return;
    loadSerialNumbers(selectedProductId);
    if (selectedType !== "in" && selectedType !== "out") {
      form.setFieldValue("type", "in");
    }
    form.setFieldValue("quantity", undefined);
    form.setFieldValue("serialNumbers", undefined);
  }, [isSerialized, selectedProductId, selectedType, loadSerialNumbers, form]);
      ...
      <ScrollShadow>
        <Form form={form} layout="vertical" onFinish={handleSubmit}>
          ...
          <Form.Item
            ...
            rules={[{ required: true, message: t`Select a product` }]}
          >
          ...
          <Form.Item name="type" label={<Trans>Type</Trans>} rules={[{ required: true }]}>
            <Select>
              <Select.Option value="in">
                <Trans>Stock in — receive goods</Trans>
              </Select.Option>
              <Select.Option value="out">
                <Trans>Stock out — consume / sell</Trans>
              </Select.Option>
              {!isSerialized && (
                <Select.Option value="adjustment">
                  <Trans>Adjustment — set count to</Trans>
                </Select.Option>
              )}
              {!isSerialized && (
                <Select.Option value="count_addition">
                  <Trans>Stock count — surplus found (add)</Trans>
                </Select.Option>
              )}
              {!isSerialized && (
                <Select.Option value="count_subtraction">
                  <Trans>Stock count — shortage found (subtract)</Trans>
                </Select.Option>
              )}
            </Select>
          </Form.Item>
          ...
          {isSerialized ? (
            <Form.Item
              ...
              label={
                selectedType === "out" ? (
                  <Trans>Serial numbers to remove</Trans>
                ) : (
                  <Trans>Serial numbers</Trans>
                )
              }
              rules={[{ required: true, message: t`At least one serial number is required` }]}
            >
              {selectedType === "out" ? (
                <Select
                  mode="multiple"
                  ...
                  options={inStockSerials.map((s: any) => ({
