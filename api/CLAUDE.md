...
- `POST /api/cash-sales/{id}/payments` — `orgRoleProtected(..., invoiceOrgID, []string{"cashbook"}, ...)` (admin/power_user/general/cashbook). Deliberately a separate route from `POST /api/payments` (accounting-tier, F104) rather than adding `cashbook` there: this one can only ever post a cash receipt into the register against one line of a customer invoice, so the till role gets exactly the write its screen needs and nothing that settles AP or voids.
