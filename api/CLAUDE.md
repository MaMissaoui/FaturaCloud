...
- `POST /api/cash-sales` — plain `protected()` with the `cashbook` role checked inline (`requireOrgRole`) — one atomic write that resolves-or-creates the client, creates the invoice, posts its GL entry, and records any upfront payment, because chaining the separate endpoints could strand an unbalanced AR entry. Counter `unitPrice`s arrive gross/tax-inclusive from the screen and are converted to net before the call. The sale ends `paid` iff `amountReceived == total`, otherwise `sent` (a loan settled later from the Cash Book, line by line — see the next route). Rejects a foreign currency (organization currency only) and a sale with no line items
- `POST /api/cash-sales/{id}/payments` — `orgRoleProtected` on the invoice's own organization with `cashbook`, and in the section guard's `cashbook` section. Settles **one invoice line** of a loan (`{invoiceLineItemId, amount, date?}`) with a cash payment into `defaultCashRegisterAccountId` (409 if unset), capped at that line's outstanding balance, and moves the invoice to `paid` in the same transaction once its whole balance clears (`db/cash_sale_payment.go`). This is the cashbook role's settlement path: `POST /api/payments` stays accounting-tier (F104), and the old Cash Book flow (`GET /api/invoices/{id}` + `POST /api/payments` + `PATCH …/state`) 403'd for this role at all three steps (audit 2026-09-24, F140). It can only receive cash against a customer invoice — never pay a vendor, void, or take a foreign-currency invoice. Any sent or paid invoice of the organization qualifies, not only one created by `POST /api/cash-sales`: there is no origin marker, and the owner decided the counter may collect any customer's open invoice (audit F150), so the Cash Book's loan tracker lists every unpaid sent invoice too
...
- `POST /api/cash-sales/{id}/payments` — `orgRoleProtected(..., invoiceOrgID, []string{"cashbook"}, ...)` (admin/power_user/general/cashbook). Deliberately a separate route from `POST /api/payments` (accounting-tier, F104) rather than adding `cashbook` there: this one can only ever post a cash receipt into the register against one line of a customer invoice, so the till role gets exactly the write its screen needs and nothing that settles AP or voids. It is not limited to Cash Book invoices (F150, by decision).
