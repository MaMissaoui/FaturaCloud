...
Schema conventions:
- Primary keys are 21-character nanoid strings — every `Create*` path guards a client-supplied empty `req.ID` with `if req.ID == "" { req.ID, _ = gonanoid.New() }` (F61, 2026-08-13 audit: `CreateInvoice` was missing this one guard, letting `{"id": ""}` insert an empty-string primary key)
