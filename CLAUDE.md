...
GET    /api/invoices/{id}/export              issue #115 — fills the org's Excel template (upload or embedded default) and returns .xlsx; ?format=pdf is accepted but always 503s in the shipped image (LibreOffice conversion isn't wired in — see db/pdf_convert.go). Not wrapped in withDB
...
- `src/routes/imports.tsx` + `src/components/imports/form.tsx` — F114 imports list with a `Drawer` form (same pattern as vendors — no detail route). The drawer's Allocation card fetches `GetImportSummary` for an existing import and shows the same committed-value/landed-cost-rate the backend derives at receiving time; the Shipment card's currency/rate fields are explicitly labelled prefill-only. `src/routes/purchase-orders/details.tsx`'s Import `Select` copies an import's currency/rate onto a **new** order only (`if (!isNew) return`, the same guard the vendor-currency cascade already uses) — an existing order's own values are never silently overwritten by re-selecting or changing its import
...
- `src/routes/invoices/details.tsx` — invoice detail/edit page; its `PDFPreview` component (F55, 2026-08-13 audit) measures its container via a `useRef` + a `useEffect` bound to that ref, not a callback ref storing cleanup on itself (which never received unmount calls, leaking the resize listener every mount); the generated object URL is tracked in a ref so each new generation revokes the previous one and unmount always revokes the current one, instead of a `[]`-deps effect closing over the initial `pdfUrl === null`. `createPDFDocument` looks up the org's PDF component via `getInvoicePDFLayout` (see `src/components/invoices/layouts.ts`) rather than importing `InvoicePDF` directly; the fiscal stamp/withholding-tax `Form.Item`s (Tunisia invoice support) are gated on `organization.fiscalStampEnabled`/`withholdingTaxEnabled`, not on `invoiceLayout` — watch for the classic `{possiblyZeroNumber && <jsx>}` footgun here (`0 && (...)` renders a literal `"0"`; both gates coerce with `!!` for exactly this reason). When `isCustomTemplateLayout(organization?.invoiceLayout)` (issue #115), the PDF/preview flow branches entirely: the live `PDFPreview`/`pdf(doc).toBlob()` path is replaced by a "no preview, export to see the result" note and an Excel button (`ExportInvoiceDocument(id, "xlsx")`) that hits the server instead of rendering client-side — no PDF button here, since PDF conversion isn't wired into the shipped image (see the Docker section) — because the server export reads persisted rows by invoice id (unlike the default/tunisia button, which reads live unsaved `form.getFieldsValue()`), the button is disabled while the form is dirty (`onValuesChange` sets a local `isDirty` flag, cleared on save) with a "save first" tooltip, rather than silently exporting a stale version
- `src/components/invoices/layouts.ts` — the invoice PDF layout registry: `invoicePDFLayouts` maps a key to a component (`default` → `pdf.tsx`, `tunisia` → `pdf-tunisia.tsx`), `getInvoicePDFLayout` falls back to `default` for a null/unrecognized `organizations.invoiceLayout` rather than failing invoice rendering, and `invoicePDFLayoutOptions` feeds the Organizations drawer's Formatting select. Adding a layout means adding both a component here and an options entry — nothing else needs to know the registry exists. `"custom"` (issue #115, "export through the org's uploaded Excel template via the server") is deliberately never added to `invoicePDFLayouts` — it has no React component — so `isCustomTemplateLayout(layoutId)` must be checked *before* calling `getInvoicePDFLayout`, which would otherwise silently fall back to `"default"` and render the wrong document next to a print button that downloads something else entirely
- `src/routes/organizations/index.tsx` — organizations list page (standalone, not under Settings); the edit drawer's Logo card (shown only for an existing org, not while creating one) is the STBvirement-style pattern: a plain `<img>` against the `/logo` URL with a local cache-busting key, not the data-URI atom the settings page and PDFs use, since this drawer can be editing an org other than the currently-selected one. The Accounting section's four Phase 7 fields (`defaultInventoryAccountId`/`defaultGRNIAccountId`/`defaultCOGSAccountId`/`defaultInventoryAdjustmentAccountId`) follow the same `Select allowClear showSearch` pattern as the existing `defaultExpenseAccountId` field, with a tooltip naming the receive/bill/ship/adjustment moment each is used at. `invoiceLayout` lives in Formatting instead (a template choice, not a GL default); `fiscalStampEnabled`/`withholdingTaxEnabled` are Checkboxes in Accounting (`valuePropName="checked"`, converted to 0/1 in `handleSubmit` — same boolean convention as `isDefault` in `src/atoms/tax-rate.ts`), and `defaultFiscalStampAmount`/`defaultStampDutyAccountId` render only while `watchedFiscalStampEnabled` is truthy, avoiding config fields for a feature the organization hasn't turned on. `defaultFiscalStampAmount` is the one money field this form edits directly (every other org money field goes through the invoice form instead), so `openEdit`/`handleSubmit` do their own cents↔units conversion rather than reusing an existing layer
...
- **Settings**: Invoice, Tax Rates, Document Templates, Backup (admin only), Users (admin only), Countries (admin only), GL Export (admin only)
...
Issue #115's custom Excel document templates were originally going to add real PDF output here (`db/pdf_convert.go` shelling out to `soffice --headless --convert-to pdf`, via `libreoffice-calc`), but that was reverted: `libreoffice-calc` crashes on startup on Alpine (`terminate called after throwing an instance of 'com::sun::star::uno::RuntimeException'`) with every standard workaround tried (`SAL_USE_VCLPLUGIN=svp`, `SAL_DISABLE_SKIA=1`, installing a JRE, installing `gcompat`) — a long-standing, unresolved Alpine/musl LibreOffice packaging fragility, not something fixable from this app's side. (Along the way, the *size* estimate for `libreoffice-calc` was also found to be wrong — an initial ~140-150MB, based on third-party examples that turned out stale for this Alpine version, was actually ~1.42GB when measured directly; that number stopped mattering once the package turned out not to run at all.) `db/pdf_convert.go`/`ConvertXLSXToPDF` is complete and tested (works wherever `soffice` actually runs, e.g. local dev with LibreOffice installed) but is **not wired into the Docker image** — `GET /api/invoices/{id}/export?format=pdf` always returns a 503 (`ErrPDFConversionUnavailable`) in the shipped image today. Custom-template export is Excel-only in the UI for this reason (see `src/routes/invoices/details.tsx`). Getting PDF working (most likely via a Debian-based runtime stage instead of Alpine, since LibreOffice runs reliably there) is a deliberately deferred follow-up, not abandoned.
